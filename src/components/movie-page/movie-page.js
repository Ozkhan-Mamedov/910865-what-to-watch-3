...
const MoviePage = (props) => {
  const {film, onFilmCardClick, authorizationStatus, activeTab, onTabClick,
    getMoreLikeThisFilm, formatRating, getTextRating, filmsComments, getStarringList,
  ...
  const _renderTab = () => {
    switch (activeTab) {
      ...
      case TabKey.REVIEWS:
        const commentsInCol = Math.ceil(filmsComments.length / 2);

        return (
          <div className="movie-card__reviews movie-card__row">
            <div className="movie-card__reviews-col">
              {filmsComments.slice(0, commentsInCol).map((comment, index) => {
                return (
            ...
            <div className="movie-card__reviews-col">
              {filmsComments.slice(commentsInCol, filmsComments.length).map((comment, index) => {
                return (
                  <div className="review" key={index}>
                    <blockquote className="review__quote">
                      <p className="review__text">{comment.comment}</p>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <MovieListWrapped films={getMoreLikeThisFilm().slice(0, MORE_LIKE_THIS_FILMS)}
